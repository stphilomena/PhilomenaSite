...
const NewMenu = () => {
    ...
    <div className="bg-gray-200 border-b-2 border-gray-400 w-screen centerAll h-10 py-5">
        ...
        <button onClick = { () => setIsOn(!isOn) }> BURGER </button>
        
        <ul className={"absolute w-80 mt-4 z-10 bg-gray-200 " + (isOn ? "" : "hidden")} >
        ...
        <div className=" flex">
            ...
            <div className={"text-gray-600  hover:text-white transition duration-300 py-2 px-3 md:px-3 lg:px-5 h-full " + (cartLength>0? "bg-red-600" : "bg-gray-300")} ><Link to="/CartPage">Cart</Link></div>
        </div>
        </div> 
        </div>
        </div>
